...
export class ImageService {
    ...
    async createImage(channelName: string, data: DatabaseResult) {
        ...
        const resultObject = values.find((obj) => obj.label === channelName);
        ...
        context.font = this.applyText(canvas, `WE ARE GODS!`, 300);
        context.fillStyle = '#ffffff';
        ...
        if (resultObject) {
            ...
            context.font = this.applyText(canvas, text, canvas.width / 3);
            // context.fillStyle = '#ffffff';
            // context.fillText(text, canvas.width / 3, canvas.height * 0.8);

            const workloadText = `${workload} `;
            context.fillStyle = this.getWorkloadColor(workload);
            ...
            const replacedChannelName = channelName.replaceAll('-', ' ');
            const text = `currently no reservations for ${replacedChannelName}!`;
            context.font = this.applyText(canvas, text, canvas.width / 3);
            context.fillText(text, canvas.width / 3, canvas.height * 0.8);
        }
    ...
    private applyText = (canvas: Canvas, text: string, width: number) => {
        const context = canvas.getContext('2d');
        let fontSize = 70;
    ...
    private getWorkloadColor = (workload: Workload) => {
        switch (workload) {
            case Workload.Low:
