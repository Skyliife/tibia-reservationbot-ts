...
class ValidationService {
    ...
    public getValidUserName(userInputName: string, guildName: string | null | undefined, interactionUserName: string) {
        ...
        if (interactionUserName && interactionUserName.trim() !== "") {
            ...
            logger.debug(`Selected Username: default`);
        return interactionUserName.trim();
    }
    ...
    public getValidReservation(reservation: UserInput, role: GuildRoles): Booking | undefined {
        ...
        logger.debug(`Role: ${role} Server Save Start: ${serverSaveStart.format()}; Server Save End: ${serverSaveEnd.format()}`);
